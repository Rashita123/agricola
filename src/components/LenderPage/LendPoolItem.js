...
export const LendPoolItem = ({ logo }) => {
  ...
  const handleLend = async () => {
    ...
    if (
      BigNumber.from(
        await instances.usdcInstance.allowance(userAddress, config.lend.address)
      ).lt(lendInput)
    )
      await instances.usdcInstance.approve(
        config.lend.address,
        ethers.constants.MaxUint256
      );
    await instances.lendInstance.lendTokens(
      BigNumber.from(lendInput).mul(10n ** 18n),
      config.usdc.address
    );
              ...
              <div class="flex flex-col w-1/2 p-2">
                <div>
                  <div class="flex flex-row justify-between">
                    ...
                    <div>
                      <Tooltip title={`${usdcBalance} USDC`} placement="top">
                        <span className="font-bold text-sm text-indigo-600 cursor-pointer">
                          Wallet Balance:{usdcBalance.slice(0, 5)}
                          {usdcBalance.length > 5 ? "..." : ""} USDC
                        </span>
                      </Tooltip>
                    </div>
                  </div>
