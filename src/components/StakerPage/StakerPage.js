...
export const StakerPage = () => {
    ...
    useEffect(() => {
        (async () => {
            if (!instances) return
            const stakedVal = normalize(await instances.lendInstance.staked(userAddress))
            setStaked(stakedVal)
            const withdrawableVal = normalize(await instances.lendInstance.withdrawableStake(userAddress))
            setWithdrawable(withdrawableVal)

            const balVal = normalize(await instances.lendInstance.balanceOf(userAddress))
            setBal(balVal)
                                    ...
                                    <dl class="mt-5 grid grid-cols-3 rounded-lg bg-white overflow-hidden shadow divide-y divide-gray-200 md:grid-cols-3 md:divide-y-0 md:divide-x">
                                        <div class="px-10 py-6">
                                            ...
                                            <dt class="mt-4 text-base font-normal text-gray-900">
                                                Staking FML helps in ensuring quality loans are approved, and stakers get to verify them to secure the network.
                                            </dt>
                                                ...
                                                <div>
                                                    <div class="flex flex-row justify-between">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span class="font-bold text-sm text-indigo-600">{bal} FML | {usdcBal} USDC</span>
                                                        </div>
                                                    </div>
