...
contract LendBorrow is ChainlinkClient, Ownable {
    ...
    function addAllowedTokens(address token) external onlyOwner {
    ...
    function withdrawTokens(uint256 _amount, address token) public {
        ...
        require(
            balance > _amount,
            "withdrawal balance cannot be less than lending balance"
        );
    ...
    function createBorrowRequest(
        ...
        userLoanIndexMapper[msg.sender].push(
            userToLoanMapper[msg.sender].length - 1
        );
        emit newLoanRequest(
            msg.sender,
            userToLoanMapper[msg.sender].length - 1,
    ...
    function approveBorrowRequest(address user, uint256 index) public {
        userToLoanMapper[user][index].noOfApprovals =
            userToLoanMapper[user][index].noOfApprovals +
            1;
    ...
    function lendMoney(uint256 index) public {
        ...
        usdcToken.transfer(
            msg.sender,
            userToLoanMapper[msg.sender][index].principal
        );
    }
}
